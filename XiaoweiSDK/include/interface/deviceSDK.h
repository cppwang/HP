...
/**
 * @brief Initialize the device SDK. A long connection to the cloud will be built, and device registration will be launched asynchronously. Waiting for notifies if you get success return(return 0). Non-zero returns imply serious errors. During the init progressing, use {@see device_check_status} to check current status.
 *
 * @param info All params are required.
 * @param notify device notify callback
 * @param init_path device SDK will write data in this path, do not delete or modify any file under this path.
 * @param auto_start_service_flag auto start service after init success. Set 0 to disable auto start. {@see AUTO_START_BIT}.
 *
 * @note DO NOT call any other API, except device_check_status, untill get success return.
 *
 * @return {@see device_error_code}.
 */
DEVICE_API int device_init(DEVICE_INFO *info, DEVICE_NOTIFY *notify, DEVICE_INIT_PATH *init_path, int auto_start_service_flag, SERVICE_CALLBACK* service_callback);
 ...
 * @return {@see device_error_code}
 */
DEVICE_API int device_get_sdk_version(unsigned int *main_ver, unsigned int *sub_ver, unsigned int *fix_ver, unsigned int *build_no);
...
#ifdef __ANDROID__
/**
 * @brief Set CA path for android https log upload.
 * @param ca_path CA file path
 * @return {@link device_error_code}
*/
DEVICE_API int device_set_ca_path(const char *ca_path);
#endif

/**
 * @brief get binder list
 * @param p_binder_list device SDK will write binder info to this location if the size of *pBinderList is enough
 * @param n_count [in]The size of *pBinderList, Too small size will result in incorrect return. [out]Real binder number.
 * @return {@see device_error_code}
 ...
 * @return {@see device_error_code}
 */
DEVICE_API int device_get_all_unverify_bind_request(DEVICE_BINDER_INFO *p_binder_verify_list, int *n_count);
 ...
 * @param jpg_filePath your head image file path, square picture, jpg type. 512x512 or 256x256 is recommended
 * @return {@see device_error_code}
 */
 ...
 * @param business_id A unique ID for the update business, same ID in another query will cancel current query. Get the business_id in xiaowie website during the register progress.
 ...
 * @return {@link device_error_code}
*/
DEVICE_API int device_query_ota_update(const char *business_id, unsigned int channel, unsigned int ota_type, unsigned int debug_flag);
 ...
 * @param ca_path If use https download, you may need to provide CA certificate file. Set nullptr to use default file.
 * @param hash_type
 *        enum HashType {
             MD5 = 1,
             SHA1 = 2,
 ...
 * @return {@link device_error_code}
*/
DEVICE_API int device_ota_download(const char *url, const char *download_dir, const char *ca_path, unsigned int hash_type, const char *hash_value);
