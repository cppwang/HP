...
typedef struct _device_info {
    /*!< Device system type. Select from these Enum {"Android", "Linux", "Windows", "IOS", "Unknown"} */
...
typedef struct _device_notify
{
     ...
     */
    void (*on_binder_list_change)(int error_code, const DEVICE_BINDER_INFO *p_binder_list, int n_count);
    
    /**
     * @brief someone want to bind this device. If your device doesn't have a screen to show his info, it is not recommended to handle this notify.
     ...
     */
    void(*on_qr_code_refresh)(int err_code, const char *qr_code_path, unsigned int expire_time, unsigned int expire_in);
...
{
    /** device will write config file, data and logs in this path. An illegal path will lead to system_err while calling device_init() */
    const char *system_path;
    
    /** device data capacity limitation. The old data will be overwrited when the data size reaches the threshold.
     The capacity must no less than 10M(system_path_capacity = 1024*1024*10), Set 0 to disable capacity limitation
     */
 ...
 * @return SDK will do nothing if reutrn 0. Return 1 to write local log file by SDK automaticly. Then xiaowei cloud can pull that log by internal command. We suggest that return 1 if log level is warn or error(3 or 4). Local log file will be auto cleaned to keep total size less than your setting(system_path_capacity).
 */
typedef void(*device_log_func)(int level, const char *tag, const char *filename, int line, const char *funcname, const char *data, unsigned int len);

/**
* @brief callback for {@see device_get_public_qr_code}.
*/
typedef void(*bind_ticket_callback)(int err_code, const char *ticket, unsigned int expire_time, unsigned int expire_in);
...
typedef struct _device_ota_callback
{
    ...
    void (*on_ota_check_result)(int err_code, const char *bussiness_id, bool new_version, OTA_UPDATE_INFO* update_info, OTA_PACKAGE_INFO* package_info);
