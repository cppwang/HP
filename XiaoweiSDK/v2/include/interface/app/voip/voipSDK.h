...
typedef struct _voip_callback
{
     ...
     * @code example
        ...
        {
            play_ring_loop("dingdingding~~~");
            time = getLocaltime();
            remark = m_contactMap.find(fromUsername)->second.remark;
            xiaowei_request_protocol_tts(voiceid, remark, time, typeVoip);
            //you can get tts"您收到来自爸爸的电话，接听还是拒绝", play this tts and wakeup, start voice request with 2V2 mode, then you can get the result
     ...
     * @brief voip handup
     */
    void (*on_handup_voip)(const char *fromIlinkid, const char *fromOpenid, const char *groupid, int hangUpType);
} VOIP_CALLBACK;
...
/**
 * @brief start voip service, this is static service, no need stop it. Re-call this interface to change VOIP_CALLBACK
 */
...
DEVICE_API int voip_send_video_data(const unsigned char *pData, int dataLen, int width, int height, int format);
DEVICE_API int voip_get_video_data(int &memberId, unsigned char *pBuf, int &dataLen, int &width, int &height);
/**
...
DEVICE_API int wechat_send_iot_msg(char *msgId, const char *username, const char *type, const char *content);
