...
#define DEF_XIAOWEI_SKILL_ID_HISTORY "8dab4796-fa37-4114-0027-7637fa2b0001"         /// skill name: 历史上的今天
...
#define DEF_XIAOWEI_SKILL_ID_IOTCTRL "8dab4796-fa37-4114-0016-7637fa2b0001"         /// skill name: 物联-物联设备控制
...
#define DEF_XIAOWEI_SKILL_ID_VOIP_NO_VEDIO "8dab4796-fa37-4114-0002-7637fa2b0001"   /// skill name: 音频通话
#define DEF_XIAOWEI_SKILL_ID_WECHAT_MSG "8dab4796-fa37-4114-0002-7637fa2b0002"      /// skill name: 通讯-微信消息
...
#define DEF_XIAOWEI_SKILL_ID_TRIGGER_ALARM "8dab4796-fa37-4114-0036-7637fa2b0001"   /// skill name: 闹钟触发场景
...
#define DEF_XIAOWEI_SKILL_ID_IOT_MASTER_DEVICE "8dab4796-fa37-4114-ffff-000000000000"       /// skill name:IOT主控
#define DEF_XIAOWEI_SKILL_ID_IOT_CONTROLLER_DEVICE "8dab4796-fa37-4114-0037-7637fa2b0001"   /// skill name:设备被控
#define DEF_XIAOWEI_SKILL_ID_STOCK "8dab4796-fa37-4114-0033-7637fa2b0001"                   /// skill name:股票

/** FM extend */
#define DEF_XIAOWEI_SKILL_ID_FM_HOT_PRO "8dab4796-fa37-4114-0024-7637fa2b0002"      /// skill name:热门节目
#define DEF_XIAOWEI_SKILL_ID_FM_TRA_PRO "8dab4796-fa37-4114-0024-7637fa2b0003"      /// skill name:传统电台
...
#define DEF_XIAOWEI_SKILL_ID_FM_XIANGSHENG "8dab4796-fa37-4114-0024-7637fa2b0009"   /// skill name:相声
...
typedef enum _xiaowei_event {
    ...
    /** Start. Triggered at the beginning of a round of requests (xiaowei_request.context.voice_request_begin == true). */
    xiaowei_event_on_request_start = 1,
    
    /** On speak. Triggered when a valid audio input detected by the cloud for the first time.
     *  > Usually the first ASR result(on_recognize event with recognized text length >0) will follow this event.
     *  > Once this event triggerd, the xiaowei_err_ai_audio_vad_time_out will not be triggered, which threshold = xiaowei_request.context.speak_timeout
     *  > Only triggered in voice request.
     */
    xiaowei_event_on_speak         = 2,
    
    /** On silent. Only triggered when the (xiaowei_request.context.request_param & xiaowei_param_local_vad == 0)
     *  > This event will triggered when the on_speak event already triggered and there is no new ASR result for a while(xiaowei_request.context.silent_timeout)
     *  > Once this event triggered, you need to stop the audio recording and stop calling xiaowei_request(calling will be droped and return errcode).
     *  > Only triggered in voice request.
     */
    ...
    /** On response. Get the final results and resources in this event (even errcode != 0)
     *  > XIAOWEI_PARAM_RESPONSE *response = (XIAOWEI_PARAM_RESPONSE *)state_info;
     *  > extend_info is an optional information in some case.
     *  > Usually only one response, but multi-response is also possible. And after all response returned, on_idle will be triggered.
     */
    ...
    /** Temp result, usually with only a TTS resource, such like "xiaowei is tring to search the song for you". */
...
typedef enum _xiaowei_param_property {
    /** Use local vad detection. In this mode, xiaowei server will not detect vad, in other words, xiaowei_event_on_silent will never be triggered. */
    xiaowei_param_local_vad         = 0x0000000000000001,
    
    /** GPS is available in this device. Once this flag is set and XIAOWEI_GPS_CALLBACK is set by xiaowei_set_gps_callback(...), xiaoweiSDK will try to get the GPS info in this callback function during request. This process will bolck the request untill the XIAOWEI_GPS_CALLBACK returns. So, make sure the callback function is fast enough, usually you should cache GPS info and read cache in this callback fucntion, rather ran call GPS driver interface here.
     */
    xiaowei_param_gps               = 0x0000000000000002,
    
    /** Do not need TTS resource. If you want to use 3-rd party TTS, set this flag. */
    xiaowei_param_no_tts            = 0x0000000000000004,
    
    /** Xiaowei official TTS package is embedded in your device, comming soon*/
...
/** @brief Device availability, some availability may not available in some case, such as in voip calling, the microphone is not for xiaowei.
 *  > Report state by xiaowei_report_state(...).
 */
...
typedef enum _xiaowei_resource_format {
    xiaowei_resource_url      = 0,    /// URL resource, your player need to support MP3, AAC, m4a, FLAC and m3u8 type.
    xiaowei_resource_text     = 1,    /// Text resource, like a story. You can display it directly or translate it to TTS to play.
    xiaowei_resource_tts_id   = 2,    /// Deprecated, TTS reource's ID (real resource will be pushed by PCM packages with opus compress (xiaowei_event_on_tts). */
    ...
    xiaowei_resource_remind   = 7,    /// Deprecated, Remind resource.
    xiaowei_resource_local    = 89,   /// Local resource (local official TTS package)
    xiaowei_resource_unknown  = 99,   /// Unknow
} XIAOWEI_RESOURCE_FORMAT;

/** @brief Resource list action type. You can get resources in response, this type shows how to insert these resources into the existing playlist (current list or history list).
 */
typedef enum _xiaowei_playlist_action {
    xiaowei_playlist_replace_all     = 0,    /// Replacing playlist. So that the current playing resource should be dropped.
    xiaowei_playlist_enqueue_front   = 1,    /// Insert into the front of playlist, keep playing current resource.
    xiaowei_playlist_enqueue_back    = 2,    /// Insert into the end of playlist, keep playing current resource.
    xiaowei_playlist_replace_enqueue = 3,    /// Keep playing current resource, and replacing detail info in playlist, trigger UI refresh.
    xiaowei_playlist_no_action       = 4,    /// Keep playing current resource, refreshing media URL in playlist.
    xiaowei_playlist_remove          = 6,    /// Removing those resouces in playlist, if current playing resource is removed, switch to next resource immediately.
...
typedef enum _xiaowei_chat_type {
    /** Voice request. In this mode, you need to call xiaowei_request many times, each time takes some voice data (64 < char_data_len < 6400). Only for the first time, context.voice_request_begin = true, and only for the last time, context.voice_request_end = true (if you use local vad, cloud vad detection no need for the end flag).
     *  In this mode, chat_data should be 16bit, 16KHz, single channel, pcm type.
     */
    ...
    /** Wakeup check request. In this mode, wakeup_type must be xiaowei_wakeup_type_cloud.
     *  This is for the device with low performance of embedded wakeup module (low accuracy). The cloud will check your wakeup words. See more detail in XIAOWEI_WAKEUP_FLAG
     *  You need to upload all audio data, including wake-up voice data.
     */
...
typedef enum _xiaowei_device_cmd {
    ...
    xiaowei_cmd_close_mic           = 1000004, /// microphone mute
    ...
    xiaowei_cmd_offset_add           = 1000017, /// seek {control_value}, unit ms. positive value is forward and negative value means rewind
    xiaowei_cmd_offset_set           = 1000018, /// seek to {control_value}, unit ms.
    xiaowei_cmd_set_play_mode        = 1000019, /// set play mode to {control_id}. 0: sequential 1: playlist loop 2: random 3: single loop
...
typedef struct _xiaowei_param_resource
{
    ...
    char *id;                          /// resource ID
    char *content;                     /// resource content. If format is URL, this is the URL, else if format is text, this is the text.
    int play_count;                    /// resource play count, usually TTS resource is 1. -1 means no limit.
    unsigned long long offset;         /// play offset, unit ms.
    char *extend_buffer;               /// extend info, json type
...
typedef enum _xiaowei_res_player_state {
    play_state_preload = 0,                 /// prepare or loading the resouce, in this state, play offset should be 0
    play_state_start = 1,                   /// start playing, in this state, play offset should be 0
    play_state_paused = 2,                  /// paused, can continue playing
    play_state_stoped = 3,                  /// stoped, usually for the resource playing finish(automaticly), if manually, use play_state_abort
...
typedef enum _xiaowei_res_play_mode {
    ...
    play_mode_shuffle_no_loop = 2,              /// shuffle playlist, finish playing while all resources are played
...
typedef struct _xiaowei_param_state
{
    ...
    XIAOWEI_AVAILABILITY availability;      /// @see XIAOWEI_AVAILABILITY
...
/** @deprecated TTS push data format */
typedef struct _xiaowei_param_audio_dat
{
    const char *id;                         /// TTS resourceID
    unsigned int seq;                       /// tts sequence
    unsigned int is_end;                    /// last package flag
    unsigned int pcm_sample_rate;           /// pcm sample rate, like 16000
    unsigned int sample_rate;               /// voice data sample rate(eg. opus)
    unsigned int channel;                   /// channel number
    XIAOWEI_AUDIO_DATA_FORMAT format;       /// format, eg. xiaowei_audio_data_opus
    const char *raw_data;                   /// raw data
    unsigned int raw_data_len;              /// raw data length
} XIAOWEI_PARAM_AUDIO_DATA;


/** @brief Login account(token) setting */
typedef struct _xiaowei_param_login_status
{
    unsigned int type;                      /// 1: QQ, 2:Wechat 3:3-rd part
    const char *app_id;                     /// appid for QQ or wechat
    const char *open_id;                    /// openId for QQ or wechat
    const char *access_token;               /// accessToken
    const char *refresh_token;              /// refreshToken
    const char *skill_id;                   /// skillId for this authorization
//    const char *third_part_extend_info;   /// 3rd-part extend info
...
typedef enum enum_xiaowei_words_type {
    /**
     * V2A list, words_list is the words array. eg. words_list[0] = "确认", words_list[1] = "取消"
     */
    xiaowei_words_command = 0,
    /**
     * contact list, words_list should be a json array, each json stands for a contact,type is:
     * {
         "id": "your unique ID",
         "name": "王缘",
         "remark": "哥哥",
     ...
     */
    xiaowei_words_contact = 1,
    /** A json words list form user */
    xiaowei_words_user_json = 2,
} XIAOWEI_WORDS_TYPE;

/**
 * Set words list result for xiaowei_set_wordslist(...), only called when the XIAOWEI_WORDS_TYPE is xiaowei_words_contact
 * @param op option 0:upload 1:delete
 * @param errCode 0:success other:failed
 */
 ...
 */
typedef void (*XIAOWEI_ON_REQUEST_CMD_CALLBACK)(const char* voice_id, int xiaowei_err_code, const char * json);
...
{
    /**
     * Callback for voice/text request, or event pushed by server(such as pick song by miniapp).
     * @param voice_id The voiceID on the request.
     * @param event {@see XIAOWEI_EVENT}
     * @param state_info auto response = reinterpret_cast<const XIAOWEI_PARAM_RESPONSE*>(state_info);
...
/**
 * @brief GPS call back. Will be called at the begin of a round of request while XIAOWEI_PARAM_PROPERTY.xiaowei_param_gps is enabled.
